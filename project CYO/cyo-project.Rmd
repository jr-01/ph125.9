...
Based on these plots, we can see that we do have a number of records with extremely low prices (in 0-5000 range) specifically 368 records. The author decided to drop these records in the dataset.
...
We apply similar filter for Z-score 3 on the "house_size" parameter, dropping another approx. 4490 rows.
...
We now look at possible correlations of our factors. As we have a combination of both numerical and categorical variables, solutions outside of what was discussed during the course are required. We use a method found in related forums [@cormatrix].
...
Prior to fitting any models, we normalize the numeric predictor values using the "scale()" function, and add price per squarefeet as response variable.
...
While we were able to derive relevant aspects of the dataset during the analysis part of the work, the prediction results - using tools and methods covered in the data science course - would be not useable for real world use cases due to poor accuracy.
...
-   possibly find advanced data science / ML techniques that yield better results with the existing dataset
